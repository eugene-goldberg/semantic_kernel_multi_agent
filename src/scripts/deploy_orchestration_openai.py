...
def load_existing_agents():
    ...
    try:
        ...
        if not chat_agent_id or not weather_agent_id:
            print("Missing agent IDs in deployment_info.json")
            return None, None
        
        print(f"Loaded agent IDs: Chat={chat_agent_id}, Weather={weather_agent_id}")
        return chat_agent_id, weather_agent_id
    except Exception as e:
        print(f"Error loading deployment info: {str(e)}")
        return None, None

def create_orchestrator_agent(client, model_name, chat_agent_id, weather_agent_id):
...
def save_deployment_info(chat_agent_id, weather_agent_id, orchestrator_id):
...
def main():
    ...
    try:
        ...
        chat_agent_id, weather_agent_id = load_existing_agents()
        
        if not chat_agent_id or not weather_agent_id:
            print("Failed to load agent IDs")
            return
        ...
        save_deployment_info(chat_agent_id, weather_agent_id, orchestrator_id)
