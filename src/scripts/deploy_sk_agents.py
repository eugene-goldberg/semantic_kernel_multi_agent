#!/usr/bin/env python3
"""
Deploy agents using Semantic Kernel SDK to Azure AI Service.
...
from src.utils.sk_deployment_manager import SkDeploymentManager, SkDeploymentSettings
...
async def deploy_single_agent(agent_type):
    ...
    try:
        ...
        SkDeploymentManager.save_deployment_info({agent_type: deployment}, f"{agent_type}_deployment_info.json")
...
async def deploy_all_agents():
    """Deploy all agents and orchestration."""
    try:
        # Create deployment settings
        settings = SkDeploymentSettings()
        ...
        deployments = await SkDeploymentManager.deploy_all_agents(settings)
        
        # Save deployment info
        SkDeploymentManager.save_deployment_info(deployments, "sk_deployment_info.json")
        ...
        logger.info(f"\nDeployment information saved to sk_deployment_info.json")
        logger.info("\nTo interact with the deployed agents, run:")
        logger.info("python src/scripts/interact_sk_agents.py")
...
def print_usage():
    ...
    print("\nUsage:")
    print("  python deploy_sk_agents.py [command]")
    print("\nCommands:")
